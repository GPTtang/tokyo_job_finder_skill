...
```bash
git clone https://github.com/your-org/tokyo_job_finder_skill ~/.claude/skills/job_finder
cd ~/.claude/skills/job_finder
pip install -e .
...
echo '\n@~/.claude/skills/job_finder/SKILL.md' >> ~/.claude/CLAUDE.md
```
...
2. Set path to `~/.claude/skills/job_finder` (or wherever you cloned it)
...
> I'm a backend engineer with Python and N2 Japanese, looking for Tokyo jobs at English-friendly or Chinese-friendly companies. Generate a `sources.json` for the job_finder skill. Include `gaijinpot`, `tokyodev`, `greenhouse`, `lever`, and `ashby` entries for 5–8 relevant companies. Each source needs `provider`, optional `token`/`company`/`board`/`url`, and a `filters` object with `keywords` and `locations`.
...
1. 打开 **设置 → 技能 → 添加技能**
2. 路径填 `~/.claude/skills/job_finder`（或你 clone 的位置）
...
> 我是有 Python 经验、日语 N2 的后端工程师，想在东京找对华人或英语友好的技术岗位。请帮我生成 job_finder 技能的 `sources.json`，包含 `gaijinpot`、`tokyodev`、`greenhouse`、`lever`、`ashby` 等 provider，针对 5～8 家合适的公司。格式：每条 source 含 `provider`，可选 `token`/`company`/`board`/`url`，以及带 `keywords` 和 `locations` 的 `filters` 对象。
...
1. **設定 → スキル → スキルを追加** を開く
2. パスに `~/.claude/skills/job_finder`（またはクローン先）を指定
...
> Python スキルと日本語 N2 を持つバックエンドエンジニアとして、東京で英語または中国語対応の技術職を探しています。job_finder スキル用の `sources.json` を生成してください。`gaijinpot`、`tokyodev`、`greenhouse`、`lever`、`ashby` を組み合わせ、5〜8 社を対象にしてください。フォーマット：各 source に `provider`、任意で `token`/`company`/`board`/`url`、`keywords` と `locations` を含む `filters` オブジェクトを含めてください。
